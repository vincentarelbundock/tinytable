...
#' @param digits Number of significant digits to keep for numeric variables. When `digits` is an integer, `tt()` calls `format_tt(x, digits = digits)` before proceeding to draw the table. Note that only the `digits` argument is passed explicitly; all other `format_tt()` arguments use their default values, which can be influenced by global options (e.g., `options(tinytable_format_num_fmt = "decimal")`). When `digits` is `NULL` (the default), `format_tt()` is not called at all, so global `tinytable_format_*` options have no effect unless the user calls `format_tt()` explicitly. Users who need more control can use the `format_tt()` function instead.
