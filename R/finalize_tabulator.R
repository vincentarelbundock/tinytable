  ...
  definition = function(x, ...) {
    ...
    if (length(x@lazy_format) > 0 || length(x@tabulator_column_formatters) > 0) {
      ...
      if (length(current_columns) > 0) {
        ...
        columns_list <- jsonlite::fromJSON(paste0("[", json_content, "]"), simplifyDataFrame = FALSE)
        ...
        for (l in x@lazy_format) {
          if (!is.null(l$j) && !is.null(l$date_format)) {
            ...
            for (col_idx in j_clean) {
              ...
              if (inherits(col_data, c("Date", "POSIXct", "POSIXlt"))) {
                formatter_js <- format_tabulator_column(x, j = col_idx, date = l$date_format)
          ...
          if (!is.null(l$j) && (!is.null(l$digits) || !is.null(l$num_mark_big) || !is.null(l$num_suffix))) {
        ...
        for (i in seq_along(columns_list)) {
          ...
          if (col_title %in% names(x@tabulator_column_formatters)) {
            ...
            formatter_obj <- jsonlite::fromJSON(x@tabulator_column_formatters[[col_title]])
            columns_list[[i]][["formatter"]] <- formatter_obj[["formatter"]]
            if (!is.null(formatter_obj[["formatterParams"]])) {
              columns_list[[i]][["formatterParams"]] <- formatter_obj[["formatterParams"]]
            }
        ...
        new_columns_json <- jsonlite::toJSON(columns_list, auto_unbox = TRUE, pretty = FALSE)
        x@table_string <- gsub(
          "columns: \\[.*?\\]",
