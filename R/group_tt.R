...
group_tt <- function(x, i, j, italic = TRUE, indent = 1) {
  ...
  if (inherits(out, "tinytable_tabularray") && missing(j)) {
    out <- group_tabularray(out, i = i, italic = italic, indent = indent)
  } else if (inherits(out, "tinytable_bootstrap") && missing(j)) {
    out <- group_bootstrap(out, i = i, italic = italic, indent = indent)
  }
