...
::: {.callout-important}
Unlike most `tinytable` operations, `subset()` is applied eagerly---it modifies the table immediately rather than deferring work to build time. For best results, call `subset()` as the **last step** in the pipeline. Calls to `style_tt()`, `format_tt()`, and `group_tt()` that appear *before* `subset()` will have their column indices automatically remapped to account for removed columns.
:::
