...
::: {.callout-warning}
`format_tt()` and `style_tt()` run only after `rbind()`/`rbind2()` finish combining the raw
tables. If headers are inserted or column types differ, the joint data is coerced to
character first, so rounding and styling operate on strings. Apply `format_tt()` to the raw
data frame before calling `tt()`, or reapply formatting/styling after binding.
:::
...
::: {.callout-important}
Unlike most `tinytable` operations, `subset()` is applied eagerly---it modifies the table immediately rather than deferring work to build time. For more predictable results, call `subset()` as the **last step** in the pipeline.

Numeric `i` and `j` indices always refer to positions in the **rendered** table, not to positions in the data frame supplied to `tt()`. Removing a column therefore does not shift the indices stored by earlier calls to `style_tt()`, `format_tt()`, or `group_tt()`: a style set on `j = 3` still applies to the third column of the printed table, and a column span still starts where it started. Indices and spans that fall outside the narrower table are clipped or dropped. To target a column by identity rather than by position, use its name (`j = "Petal.Width"`) instead of a number. Conditional styling based on column *values* (`i = Species == "versicolor"`) is evaluated against the data, so it keeps tracking the same rows even after the column it references is dropped.
:::
