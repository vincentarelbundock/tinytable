...
Quarto automatically applies some data processing to the content of the tables it renders. By default, `tinytable` disables this processing, because it can enter in conflict with styling and formatting features of the package. To enable Quarto data processing, users can set a global option:

```{r}
options(tinytable_quarto_disable_processing = FALSE)
```

This option then allows users to mark certain cells explicitly for processing by Quarto, by wrapping them in a special "span" called "data-qmd", defined by Quarto: 

```{r}
k <- data.frame(Thing = "x^2^", Citation = "@Lovelace1842")
fn <- function(z) sprintf("<span data-qmd='%s'></span>", z)
tt(k) |> format_tt(fn = fn)
```
