...
::: {.callout-warning}
The `subset()` method works in simple use-cases does not always adjust all the internal indices used for styling and grouping. For complex tables, it is safer to select columns and rows before calling `tt()`, and to supply vector indices to the various styling and grouping functions, rather than relying on column names.
:::
