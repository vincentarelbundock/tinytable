...
*Note: Whole-table rotation is only supported in LaTeX and Typst output.*
:::
...
Warning: The substitution code is very simple and it may not work properly when there are unmatched `$` symbols in the document. You may prefer to use `#mi()` instead of `#mitex()` to get different alignment.
