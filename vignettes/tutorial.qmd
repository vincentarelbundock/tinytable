...
The `style_bootstrap()` function allows us to declare CSS properties and values for individual cells, columns, or rows of a table. For example, if we want to make the first column bold, we could do:

```{r}
tt(x) |> style_bootstrap(j = 1, css = "font-weight: bold; color: red;")
```

## CSS rules

For more extensive customization, we can also define complete CSS rules. In this example, we define several rules that should apply to a new class called `mytable`. Then, we use the `theme` argument of the `tt()` function to make sure that the tiny table is of class `mytable`. Finally, we call `style_bootstrap()` with the `css_rule` argument.
...
tt(x, theme = "table mytable") |> 
  style_bootstrap(css_rule = css_rule)
```
