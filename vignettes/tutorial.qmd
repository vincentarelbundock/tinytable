...
```{r}
tt(x) |> style(2, c(1, 3), background = "olive")
```
...
```{r}
tt(x) |> style(background = "black", color = "white", bold = TRUE)
```
...
```{r}
tt(x) |> style(i = 1:2, color = "blue")
```

```{r}
tt(x) |> style(j = c(2, 4), bold = TRUE)
```
...
```{r}
tt(x, html = bootstrapOptions(class = "table table-dark"))
```

We can also combine several Bootstrap classes. Here, we get a "striped" table with the "warning" color:

```{r}
tt(x, html = bootstrapOptions(class = "table table-striped table-warning"))
```
...
```{r}
tt(x, latex = bootstrapOptions(theme = "void", hlines = ""))
```
