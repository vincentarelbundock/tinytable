...
tt(iris) |>
  theme_latex(
    multipage = TRUE, 
    rowhead = 1,
    outer = "caption={Example table.}, label={tbl-ex-multipage}")
```

Unfortunately, this approach means that the standard Quarto cross-reference syntax will no longer work, so users must use LaTeX syntax directly instead:

```latex
\autoref{tbl-ex-multipage}
