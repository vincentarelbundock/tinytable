...
The Bootstrap framework provides a number of built-in themes to style tables, using "classes." To use them, we call `theme_html()` with the `class` argument. A list of available Bootstrap classes can be found here: <https://getbootstrap.com/docs/5.3/content/tables/>
...
lm(mpg ~ hp, data = mtcars) |> tt(digits = 3)

lm(mpg ~ hp + factor(cyl), data = mtcars) |> tt(digits = 3)
```
