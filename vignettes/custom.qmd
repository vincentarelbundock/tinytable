...
The Bootstrap framework provides a number of built-in themes to style tables, using "classes." To use them, we call `theme_tt("bootstrap", ...)` with the `bootstrap_class` argument. A list of available Bootstrap classes can be found here: <https://getbootstrap.com/docs/5.3/content/tables/>

To produce a more condensed or "small" table, we use the `table-sm` class:

```{r, eval = knitr::is_html_output()}
tt(x) |> theme_tt("bootstrap", bootstrap_class = "table table-sm")
```
...
tt(head(iris), caption = "A caption on top") |>
  theme_tt("bootstrap", bootstrap_class = "table caption-top")
```
...
::: {.content-visible when-format="html"}
The `theme_tt("bootstrap", ...)` function allows us to declare CSS properties and values for individual cells, columns, or rows of a table. For example, if we want to make the first column bold, we could do:

```{r}
tt(x) |>
  theme_tt("bootstrap", j = 1, bootstrap_css = "font-weight: bold; color: red;")
```
:::
...
tt(x, width = 2 / 3) |>
  ...
  theme_tt(bootstrap_class = "table mytable", bootstrap_css_rule = css_rule)
```
...
tt(head(iris), caption = "Hello world") |>
  theme_tt("bootstrap", bootstrap_class = "table bottomcaption", bootstrap_css_rule = rule)
```
