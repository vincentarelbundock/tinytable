...
For more extensive customization, we can supply a complete CSS rule for the `.tinytable` class. To do so, we use the `theme_html()` function and its `css_rule` argument. 
...
2. We assign a custom class name to the table using the `class` argument in `theme_html()`, otherwise styles will spill over to other tables in the same document.
...
```{r, eval = knitr::is_html_output()}
tt(x) |> theme_html(engine = "bootstrap", class = "table table-sm")
```
