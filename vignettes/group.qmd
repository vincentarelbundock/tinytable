...
Calculating the location of rows can be cumbersome. Instead of doing this by hand, we can use the "group" shortcut to style rows. 

```{r}
library(magrittr)
tt(df) %>%
  group_tt(i = df$am) %>%
  style_tt(i = "group", background = "pink")
```
...
Normally, we would call `style_tt(i = "group")` to style the row groups, but here we need the actual indices to also style one row below the groups. We can use the `@group_index_i` slot to get the indices of the row groups.
