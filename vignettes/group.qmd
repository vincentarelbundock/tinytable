...
```{r}
tab |> 
  style_tt(i = "groupi", color = "white", background = "teal") |>
  style_tt(i = "~groupi", j = 1, indent = 2)
```
...
tt(x) |>
  group_tt(j = list("Foo" = 2:3, "Bar" = 5)) |>
  group_tt(j = list("Hello" = 1:2, "World" = 4:5)) |>
  style_tt(i = 0, color = "#E69F00") |>
  style_tt(i = -1, color = "#56B4E9") |>
  style_tt(i = -2, color = "#F0E442")
```
...
* test error on no group_tt(j). 
* Test distinction between colnames and groupj. 
* test i="colnames" when there are no column names
