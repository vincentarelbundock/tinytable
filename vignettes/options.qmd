...
* EXPERIMENTAL `options(tinytable_save_html_mathjax = TRUE)` inserts MathJax scripts in the HTML document. Warning: This may conflict with other elements of the page if MathJax is otherwise loaded.
...
* `options(tinytable_save_markdown_hlines = TRUE)` sets whether gridlines are included in markdown tables. This should be true for `docx` conversion.
...
* `options(tinytable_save_disable_quarto_processing = TRUE)` disables Quarto processing of cell content. 
