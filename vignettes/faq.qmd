...
In some contexts, users may want to include multi-line cells or complex Markdown formatting in their tables. The key distinction here is that one-line Markdown can often be rendered directly with `markdown = TRUE` in `tinytable`. However, when you have multi-line Markdown (e.g., bullet points, lists, or long text), you need to wrap it in a LaTeX `\minipage` environment so that it renders correctly in PDF output. To do this, we write a simple helper functions: `minipagify`.
