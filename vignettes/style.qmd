...
2. A specialized interface which allows users to use the [powerful `tabularray` package](https://ctan.org/pkg/tabularray?lang=en) to customize LaTeX tables. This is accessed by passing `tabularray` settings as strings to the `inner` and `outer` arguments of `theme_latex()`.
...
The `style_tt` function allows us to customize the borders that surround eacell of a table, as well horizontal and vertical rules. To control these lines, we use the `line`, `line_width`, `line_color`, and `line_type` arguments. Here's a brief overview of each of these arguments:

- `line`: This argument specifies where lines should be drawn. It is a string that can consist of the following characters:
...
- `line_width`: This argument controls the width of the lines in em units (default: 0.1 em). You can adjust this value to make the lines thicker or thinner.
- `line_color`: Specifies the color of the lines. You can use color names, hexadecimal codes, or other color specifications to define the line color.
- `line_type`: Specifies the type of line: `"solid"` (default), `"dashed"`, or `"dotted"`. Markdown and Word ignore this argument and always draw solid lines.
...
Lines can also be dashed or dotted:

```{r}
tt(x, theme = "empty") |>
  style_tt(i = 1, line = "b", line_type = "dashed") |>
  style_tt(i = 2, line = "b", line_type = "dotted", line_color = "orange")
```
...
![ANSI Terminal Output](figures/ansi_example.png)
