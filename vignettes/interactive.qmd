...
The `style_tt()` function supports comprehensive styling for Tabulator tables, including bold, italic, color, background, fontsize, and more. Styles are applied with cell-level precision and persist across sorting and pagination.

```{r}
tt(iris) |>
  theme_html(engine = "tabulator", tabulator_pagination = c(10, 50)) |>
  style_tt(i = seq(1, 150, by = 5), j = 2, background = "black", color = "white")
```
...
Here's an example that applies a gradient color scale to a column, dynamically adjusting text color for readability:
...
tt(iris) |>
    theme_html(engine = "tabulator", tabulator_pagination = c(25, 50, 150)) |>
    style_tt(
        i = 1:nrow(iris),
        j = 1,
        align = "c",
        color = ifelse(iris[[1]] < quantile(iris[[1]], .8), "white", "black"),
        background = gradient(iris[[1]]))
```
