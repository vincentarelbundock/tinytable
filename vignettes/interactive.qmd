...
::: {.callout-note}
To use a theme, the HTML file must load a style sheet *globally* in the document. Unfortunately, this means that `tinytable` cannot apply a `Tabulator` style sheet to a single table in documents with multiple table. The style that applies is always the last one loaded in the document.
:::
