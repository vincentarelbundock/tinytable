...
* LaTeX: Fix Windows path issue where backslashes and absolute paths in `\includegraphics` caused compilation failures. Now uses relative paths with forward slashes on all platforms (Issue #620, thanks to @morganwkelly and @resulumit; Issue #630, thanks to @marklhc).
...
* `theme_rotate()` now applies to HTML output and supports `i`/`j` for rotating cell content.
